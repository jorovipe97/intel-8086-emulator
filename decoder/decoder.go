...
func (deco *Decoder) NextInstruction() (Instruction, error) {
	if !deco.memory.HasMoreInstructions() {
	...
	// Move current instruction pointer after deconding the instruction and before executing it
	deco.memory.IncrementPosition(result.Size)
...
func (deco *Decoder) tryDecode(candidateInstruction InstructionEncoding) (Instruction, error) {
	...
	// Remember this is a try decode we cannot keep moving current AbsolutePosition on each tryDecode
	// all tryies should start from the same byte.
	internalPosition := &internalPos{
		pos: deco.memory.AbsolutePosition,
	}
	...
	for _, testBits := range candidateInstruction.bits {
		...
		// Due to the nature of go zero initialize everything. Firts bit
		...
		if testBits.BitCount != 0 {
			if bitsPendingCount == 0 {
				...
				bitsPending, err = deco.memory.GetByteAtPosition(internalPosition.pos)
				if err != nil {
					return Instruction{}, fmt.Errorf("error when trying to decode: %w", err)
				}
	...
	if valid {
		...
		result.Size = internalPosition.pos - deco.memory.AbsolutePosition
		result.RawBits = deco.memory.Data[deco.memory.AbsolutePosition:internalPosition.pos]
...
func (deco *Decoder) parseDispValue(currentPosition *internalPos, mod int, dispIsW bool) int {
	...
	if dispIsW {
		...
		disp0 := int(deco.memory.Data[currentPosition.pos])
		disp1 := int(deco.memory.Data[currentPosition.pos+1])
		...
		// Then cast to int so go compiler performs a sign extension.
		displacement = int(int8(deco.memory.Data[currentPosition.pos]))
...
func (deco *Decoder) parseDataValue(currentPosition *internalPos, hasData, dataIsW bool) int {
	...
	if hasData {
		if dataIsW {
			data0 := int(deco.memory.Data[currentPosition.pos])
			data1 := int(deco.memory.Data[currentPosition.pos+1])
			// Perform cast to sign 16 so we get symbol correctly.
			// Then cast to int so go compiler performs a sign extension.
			...
			// Perform cast to sign 8 so we get symbol correctly.
			// Then cast to int so go compiler performs a sign extension.
			data = int(int8(deco.memory.Data[currentPosition.pos]))
			currentPosition.pos += 1
		}
	}
